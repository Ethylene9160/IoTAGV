...
int mpu_set_bypass(unsigned char bypass_on)
{
    ...
    if (bypass_on) {
        if (i2c_read(st.hw->addr, st.reg->user_ctrl, 1, &tmp)) {
            // 串口输出错误消息
            char error_msg[] = "Error: Failed to read user_ctrl register\n";
        ...
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &tmp)) {
            // 串口输出错误消息
            char error_msg[] = "Error: Failed to write user_ctrl register\n";
            HAL_UART_Transmit(&huart2, (uint8_t*)error_msg, sizeof(error_msg) - 1, HAL_MAX_DELAY);
            return -1;
        }
        delay_ms(3);
        ...
        if (i2c_write(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp)) {
            // 串口输出错误消息
            char error_msg[] = "Error: Failed to write int_pin_cfg register\n";
            HAL_UART_Transmit(&huart2, (uint8_t*)error_msg, sizeof(error_msg) - 1, HAL_MAX_DELAY);
            return -1;
        }
    } else {
    ...
    // 成功设置模式时输出调试信息
    if (bypass_on) {
        char debug_msg[] = "MPU bypass mode set to ON\n";
        HAL_UART_Transmit(&huart2, (uint8_t*)debug_msg, sizeof(debug_msg) - 1, HAL_MAX_DELAY);
    } else {
        char debug_msg[] = "MPU bypass mode set to OFF\n";
        HAL_UART_Transmit(&huart2, (uint8_t*)debug_msg, sizeof(debug_msg) - 1, HAL_MAX_DELAY);
    }
...
static int setup_compass(void)
{
    ...
    if(i2c_read(st.chip_cfg.compass_addr, AKM_REG_ASAX, 3, data) != HAL_OK) {
        char error_msg[] = "I2C read error\n";
        HAL_UART_Transmit(&huart2, (uint8_t*)error_msg, sizeof(error_msg) - 1, HAL_MAX_DELAY);
    }
    ...
    char uart_msg[50];
    sprintf(uart_msg, "Received data: %d, %d, %d\n", data[0], data[1], data[2]);
