...
void Tag_Measure_Dis(void)
{
    ...
    for(dest_anthor = 0 ;  dest_anthor<ANCHOR_MAX_NUM; dest_anthor++)
    {
        ...
        tx_poll_msg[ALL_MSG_TAG_IDX] = TAG_ID;//基站收到标签的信息，里面有TAG_ID,在基站回复标签的时候，也需要指定TAG_ID,只有TAG_ID一致才做处�?
   ...
   // dwt_dumpregisters();
            ...
            if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
            {
                ...
                if (status_reg & SYS_STATUS_RXFCG)
                {
                    ...
                    if (memcmp(rx_buffer, rx_final_msg, ALL_MSG_COMMON_LEN) == 0)
                    {
                        ...
                        // a=x;  //自动类型转换，取整数部分
                        distance_msg[11] = temp%100;  //�?100后对100取余，得�?2位小数点后数�?
    ...
    while(1)
    {
        ...
        if(TAG_ID == MASTER_TAG)//master  tag
        {
            ...
            //可能存在的问题，TAG收到Semaphore 没有释放就离开网络，导致Master TAG无法收回Semaphore，这个需要定时器实现，定时一段时间，若依然没有收到TAG 释放Semaphore，需要强制取�?
...
// CRC16计算函数（CRC-CCITT-FALSE）
uint16_t calculate_crc16(const uint8_t *data, size_t length) {
    uint16_t crc = 0xFFFF; // 初始化CRC值
    uint16_t poly = 0x1021; // 使用的多项式
...
void float_to_bytes(float value, uint8_t *bytes_array) {
    union {
        float f;
        uint8_t bytes[4];
    } float_union;
    float_union.f = value;
    memcpy(bytes_array, float_union.bytes, 4);
}


void send_data(uint16_t self_id, uint16_t target_id, float dist1, float dist2) {
    uint8_t buffer[19]; // 根据协议字段长度计算
    size_t index = 0;

    buffer[index++] = 0x5A;
    buffer[index++] = self_id & 0xFF; // self_id 低位
    buffer[index++] = (self_id >> 8) & 0xFF; // self_id 高位
    buffer[index++] = target_id & 0xFF; // target_id 低位
    buffer[index++] = (target_id >> 8) & 0xFF; // target_id 高位
    size_t crc8_index = index;
    buffer[index++] = 0;
    buffer[index++] = 8; // dist1和dist2各4字节，总共8字节
    buffer[index++] = 0; // 补充长度为两位，低位补充0
    float_to_bytes(dist1, &buffer[index]);
    index += 4;
    float_to_bytes(dist2, &buffer[index]);
    index += 4;

    buffer[crc8_index] = calculate_crc8(buffer, index);

    uint16_t crc16 = calculate_crc16(buffer, index);
    buffer[index++] = crc16 & 0xFF;
    buffer[index++] = (crc16 >> 8) & 0xFF;
...
static void distance_mange(void)
{
    ...
    uint8_t target_id = 0x02;
    float dist1 = 0.0f; // 初始化dist1
    float dist2 = 0.0f; // 初始化dist2
    char dist_str[20];
    if(Anthordistance_count[0]>0)
    {
        sprintf(dist_str, "an0:%3.2fm", (float)Anthordistance[0]/1000);       
        OLED_ShowString(0, 2,dist_str);
		dist1 = (float)Anthordistance[0] / 1000;
		}
    ...
    if(Anthordistance_count[1]>0)
    {
		...
		dist2 = (float)Anthordistance[1]/1000;
    }
	send_data(self_id, target_id, dist1, dist2);
}


#define DISTANCE3 0.20
